...
def upgrade() -> None:
    op.add_column("functions", sa.Column("canary_version_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column("functions", sa.Column("canary_percent", sa.Integer(), nullable=False, server_default="0"))
    ...
    op.create_table(
        ...
        sa.Column("function_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("functions.id", ondelete="CASCADE"), nullable=False),
        sa.Column("actor_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=True),
        sa.Column("previous_version_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("function_versions.id", ondelete="SET NULL"), nullable=True),
        sa.Column("activated_version_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("function_versions.id", ondelete="SET NULL"), nullable=True),
        sa.Column("canary_version_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("function_versions.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_index("ix_version_activation_function_created", "version_activations", ["function_id", "created_at"])
...
def downgrade() -> None:
    ...
    op.drop_constraint("fk_functions_canary_version_id_function_versions", "functions", type_="foreignkey")
