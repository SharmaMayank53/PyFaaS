...
class TestFunctionCRUD:
    ...
    async def test_get_function(
        ...
        resp = await client.get(f"/api/v1/functions/{test_function.id}", headers=auth_headers)
        assert resp.status_code == 200
        assert resp.json()["id"] == str(test_function.id)

    async def test_get_function_not_found(self, client: AsyncClient, auth_headers: dict) -> None:
        import uuid

        resp = await client.get(f"/api/v1/functions/{uuid.uuid4()}", headers=auth_headers)
    ...
    async def test_delete_function(
        ...
        # Verify deleted
        resp2 = await client.get(f"/api/v1/functions/{test_function.id}", headers=auth_headers)
    ...
    async def test_delete_function_not_found(
        ...
        resp = await client.delete(f"/api/v1/functions/{uuid.uuid4()}", headers=auth_headers)
        assert resp.status_code == 404
    ...
    async def test_delete_function_with_related_records(
        ...
        assert (
            await db_session.execute(select(func.count()).select_from(FunctionVersion))
        ).scalar_one() == 0
        assert (
            await db_session.execute(select(func.count()).select_from(Schedule))
        ).scalar_one() == 0
        assert (
            await db_session.execute(select(func.count()).select_from(VersionActivation))
        ).scalar_one() == 0

        remaining_execution = (
            await db_session.execute(select(Execution).where(Execution.id == execution.id))
        ).scalar_one()
        ...
        assert (
            await db_session.execute(select(func.count()).select_from(ExecutionLog))
        ).scalar_one() == 1
